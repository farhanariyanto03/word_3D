...
@section('content')
    <div class="p-4">
        <div class="flex justify-between items-center mb-8">
            <h2 class="text-xl font-semibold text-gray-800 dark:text-white">Daftar Nomor Rekening</h2>
            <a href="{{ route('nomor-rekening.create') }}"
                ...
                <tbody>
                    @foreach ($norek as $n)
                        <tr class="bg-white dark:bg-gray-800 border-b hover:bg-gray-50 dark:hover:bg-gray-700">
                            ...
                            <td class="px-6 py-5 leading-relaxed">{{ $n->atas_nama }}</td>
                            <td class="px-6 py-5 leading-relaxed">{{ $n->nama_bank }}</td>
                            <td class="px-6 py-5 leading-relaxed">{{ $n->nomor_rekening }}</td>
                            <td class="px-6 py-5 leading-relaxed flex flex-row space-x-1">
                                <a href="{{ route('nomor-rekening.edit', Crypt::encrypt($n->id)) }}"
                                    class="px-4 py-3 text-white bg-yellow-500 rounded-lg hover:bg-yellow-600">Edit</a>
                                <form action="{{ route('nomor-rekening.destroy', Crypt::encrypt($n->id)) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
